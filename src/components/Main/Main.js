...
const Main = () => {
  ...
  useEffect(() => { // sem ação se o valor do input for vazio
    if (inputHeader === "") {
      return; 
    }
    fetch(
      `http://localhost:3000/artists?name_like=${inputHeader}`
    )
      .then((res) => res.json())
  ...
  // Dentro da função shareDados, o valor do input é obtido a partir de e.target.value (onde E é o evento do input).
